...
All project rules, workflows, and agent templates live in `_qsprocess_opencode/`. Tool-specific config (`.opencode/`) references `_qsprocess_opencode/`, never duplicates it.
...
Each skill is defined in `_qsprocess_opencode/` and `.opencode/agents/` and handles all steps including quality gates.
...
Before implementing code, read `_qsprocess_opencode/project-context.md` for the full 42-rule set covering naming, async, logging, error handling, and testing patterns.
