...
class QSCTest(HADeviceMixin, AbstractLoad):
    def __init__(self, **kwargs):
        ...
        self.attach_ha_state_to_probe(
            self.fake_id_1, is_numerical=True, non_ha_entity_get_state=self.fake_id_1_state_getter
        )

        self.attach_ha_state_to_probe(
            self.fake_id_2, is_numerical=True, non_ha_entity_get_state=self.fake_id_2_state_getter
        )
...
class TestDeviceUtils(TestCase):
    def test_utils(self):
        ...
        val1_no_invalid = device.get_state_history_data(
            device.fake_id_1, num_seconds_before=200, to_ts=last, keep_invalid_states=False
        )
        ...
        val2_no_invalid = device.get_state_history_data(
            device.fake_id_2, num_seconds_before=200, to_ts=last, keep_invalid_states=False
        )
        ...
        val1_all = device.get_state_history_data(
            device.fake_id_1, num_seconds_before=200, to_ts=last, keep_invalid_states=True
        )
        ...
        val2_all = device.get_state_history_data(
            device.fake_id_2, num_seconds_before=200, to_ts=last, keep_invalid_states=True
        )
        ...
        assert (
            int(a_no) == 3574
        )  # before was ... but changed with time based calculus np.mean([v for _, v, _ in sum_no_invalid_res])
