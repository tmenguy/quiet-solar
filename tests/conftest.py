...
class FakeConfigEntries:
    ...
    def async_entries(self, domain: str) -> list[FakeConfigEntry]:
        ...
        return [entry for entry in self.hass.data.get(domain, {}).values() if isinstance(entry, FakeConfigEntry)]
...
def mock_config_entry():
    ...
    return FakeConfigEntry(entry_id="test_entry_123", data={CONF_NAME: "Test Device"}, title="Test Device")
...
def mock_external_apis():
    ...
    with patch(
        "custom_components.quiet_solar.ha_model.home.QSHome.update_forecast_probers", new_callable=AsyncMock
    ) as mock_forecast:
        yield {
...
class MockLazyState:
    ...
    def __init__(
        self, entity_id: str, state: str, attributes: dict | None = None, last_updated: datetime | None = None
    ):
        from datetime import datetime
...
def mock_lazy_state_factory():
    ...
    def factory(entity_id: str, state: str, attributes: dict | None = None, last_updated: datetime | None = None):
...
def mock_gps_states(mock_lazy_state_factory):
    ...
    def create_gps_path(entity_id: str, locations: list, start_time: datetime | None = None):
        ...
        for state_name, lat, lon, minutes in locations:
            ...
            state = mock_lazy_state_factory(entity_id, state_name, {"latitude": lat, "longitude": lon}, time)
            states.append(state)
        return states
