...
class TestSolverEnergyConservation:
    ...
    def test_battery_charging_full_day_validates_soc(self):
        ...
        pv_forecast = build_realistic_solar_forecast(start_time=start_time, num_hours=24, peak_power=8000.0)

        # Realistic consumption
        ua_forecast = build_realistic_consumption_forecast(
            start_time=start_time, num_hours=24, base_load=500.0, peak_evening_load=1500.0, night_load=300.0
        )
        ...
        battery_charge = reconstruct_battery_charge_evolution(
            initial_charge=battery._current_charge_value, battery_power=battery_power, durations_s=durations_s
        )

        validate_battery_soc_bounds(battery_charge=battery_charge, battery=battery, tolerance_wh=1.0)
        ...
        print("✅ Battery full day cycle test passed!")
    ...
    def _scenario_high_solar_no_battery(self):
        ...
        car_steps = [LoadCommand(command="ON", power_consign=a * 230 * 3) for a in range(7, 17)]
    ...
    def _scenario_off_grid(self):
        ...
        battery = create_test_battery(capacity_wh=8000.0, initial_soc_percent=70.0, min_soc_percent=20.0)
    ...
    def test_battery_charge_discharge_cycle_validation(self):
        ...
        battery = create_test_battery(
            capacity_wh=10000.0, initial_soc_percent=50.0, min_soc_percent=15.0, max_soc_percent=95.0
        )
        ...
        print("✅ Battery charge/discharge cycle validated!")
    ...
    def test_battery_inverter_clamping_limits_ac_output(self):
        ...
        battery = create_test_battery(capacity_wh=15000.0, initial_soc_percent=40.0, max_charge_power=6000.0)
        ...
        print("✅ Inverter clamping test passed!")
        print(f"   - Max AC output respected: {max_inverter_dc_to_ac_power}W")
        print("   - Battery absorbed excess")
    ...
    def test_solver_with_empty_battery_prevents_overdischarge(self):
        ...
        assert heater_energy < 10000, (
            f"Optional load should be limited when battery low\n  Delivered: {heater_energy:.1f}Wh\n  Target: 10000Wh"
        )

        # Energy conservation validated by with_self_test=True

        print("✅ Battery minimum SOC protection validated!")
...
class TestSolverBatterySegmentation:
    ...
    def test_battery_segmentation_prevents_empty_periods(self):
        ...
        print("✅ Battery segmentation prevents empty periods!")
        print(f"   - Minimum SOC reached: {100 * min_soc_reached / battery.capacity:.1f}%")
        print("   - Battery never empty: ✓")
