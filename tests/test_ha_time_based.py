...
async def test_load_update_interval(
    ...
    with patch("custom_components.quiet_solar.ha_model.home.QSHome.update_forecast_probers", new_callable=AsyncMock):
        with patch(
            "custom_components.quiet_solar.ha_model.home.QSHome.update_loads", new_callable=AsyncMock
        ) as mock_update_loads:
...
async def test_state_update_interval(
    ...
    with patch("custom_components.quiet_solar.ha_model.home.QSHome.update_forecast_probers", new_callable=AsyncMock):
        with patch(
            "custom_components.quiet_solar.ha_model.home.QSHome.update_all_states", new_callable=AsyncMock
        ) as mock_update_states:
            await hass.config_entries.async_setup(config_entry.entry_id)
            await hass.async_block_till_done()
