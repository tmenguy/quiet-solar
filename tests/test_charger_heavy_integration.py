...
def create_mock_car(name="TestCar", charge_percent=50.0, battery_capacity=60000):
    ...
    car.get_charge_power_per_phase_A = MagicMock(
        return_value=([0] * 6 + [1380, 1610, 1840, 2070, 2300, 2530, 2760, 2990, 3220, 3450, 3680] + [3680] * 20, 6, 32)
    )
...
def create_mock_charger_status(charger, current_amp=10, num_phases=3, score=100):
    ...
    # Mock methods
    cs.get_current_charging_amps = MagicMock(
        return_value=[current_amp] * num_phases if num_phases == 3 else [current_amp, 0, 0]
    )
...
class TestCheckLoadActivityAndConstraints:
    ...
    async def test_plugged_attaches_best_car(self, setup_charger):
        ...
        from custom_components.quiet_solar.home_model.commands import CMD_AUTO_GREEN_ONLY
    ...
    async def test_force_charge_creates_fast_constraint(self, setup_charger):
        ...
        with (
            ...
            patch.object(charger, "push_live_constraint", return_value=(True, False)) as mock_push,
...
class TestDynHandle:
    ...
    async def test_handles_remaining_budget(self, setup_charger_group):
        ...
        with (
            patch.object(charger_group, "ensure_correct_state", new_callable=AsyncMock) as mock_ensure,
...
class TestBudgetingAlgorithm:
    ...
    async def test_single_charger_budgeting(self, setup_charger_group):
        ...
        with (
            patch.object(charger_group, "_do_prepare_and_shave_budgets", new_callable=AsyncMock) as mock_prep,
...
class TestChargerGroupEnsureCorrectState:
    ...
    async def test_sorts_by_score(self, setup_charger_group):
        ...
        with (
            patch.object(chargers[0], "ensure_correct_state", new_callable=AsyncMock) as mock_ensure1,
...
class TestPrepareAndShaveBudgets:
    ...
    async def test_reset_allocation_uses_minimum(self, setup_charger_group):
        ...
        with (
            patch.object(charger_group, "_shave_mandatory_budgets", new_callable=AsyncMock) as mock_shave_mand,
