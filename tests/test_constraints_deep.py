...
class TestConstraintEnergyConversion:
    ...
    def test_multisteps_energy_time_conversion_roundtrip(self):
        ...
        assert abs(time_s - expected_time_s) < 1.0, (
            f"Time calculation incorrect\n  Calculated: {time_s}s\n  Expected: {expected_time_s}s"
        )

        print("✅ Energy/time conversion test passed!")
    ...
    def test_charge_percent_conversion_with_capacity(self):
        ...
        power_steps = [LoadCommand(command="ON", power_consign=a * 230 * 3) for a in range(7, 17)]
        ...
        assert abs(delta_energy - expected_delta) < 1.0, (
            f"Delta calculation incorrect\n  60% delta converted to: {delta_energy}Wh\n  Expected: {expected_delta}Wh"
        )

        print("✅ ChargePercent conversion test passed!")
    ...
    def test_time_based_constraint_duration_calculation(self):
        ...
        assert abs(time_s - target_seconds) < 0.1, (
            f"Time-based time conversion incorrect\n  Input: {target_seconds}s\n  Output: {time_s}s"
        )

        # Roundtrip
        back = constraint.convert_time_to_target_value(time_s)
        assert abs(back - target_seconds) < 0.1, f"Time roundtrip failed\n  {target_seconds}s → {time_s}s → {back}s"
        ...
        assert abs(duration.total_seconds() - target_seconds) < 1.0, (
            f"best_duration_to_meet incorrect\n  Returned: {duration.total_seconds()}s\n  Expected: {target_seconds}s"
        )

        print("✅ Time-based constraint calculations passed!")
...
class TestConstraintAdaptRepartition:
    ...
    def test_adapt_repartition_adds_energy_to_slots(self):
        ...
        assert 200 <= energy_added <= 400, (
            f"Energy added doesn't match request\n  Requested: {energy_delta}Wh\n  Added: {energy_added:.1f}Wh"
        )
        ...
        assert abs(remaining_delta) <= abs(energy_delta), (
            f"Remaining delta larger than initial\n  Initial: {energy_delta}Wh\n  Remaining: {remaining_delta}Wh"
        )
        ...
        print("✅ adapt_repartition add energy test passed!")
    ...
    def test_adapt_repartition_reduces_energy_from_slots(self):
        ...
        power_slots_duration_s = np.array([SOLVER_STEP_S] * 4, dtype=np.float64)
        existing_commands = [LoadCommand(command="ON", power_consign=1000.0) for _ in range(4)]
        ...
        print("✅ adapt_repartition reduce energy test passed!")
    ...
    def test_adapt_repartition_respects_allow_change_state(self):
        ...
        assert out_commands[1] is None or out_commands[1].power_consign == 0, (
            "Slot 1 should stay OFF when allow_change_state=False"
        )

        assert out_commands[3] is None or out_commands[3].power_consign == 0, (
        ...
        if has_changes:
            ...
            slot_0_increased = out_commands[0] and out_commands[0].power_consign > existing_commands[0].power_consign
            slot_2_increased = out_commands[2] and out_commands[2].power_consign > existing_commands[2].power_consign

            assert slot_0_increased or slot_2_increased, "At least one already-ON slot should be increased"

        print("✅ allow_change_state test passed!")
        print("   - OFF slots stayed OFF: ✓")
        print("   - ON slots could be adjusted: ✓")
...
class TestConstraintComputeBestPeriod:
    ...
    def test_compute_best_period_as_fast_as_possible(self):
        ...
        power_available_power = np.array([-6000.0, -6000.0, -6000.0, -2000.0, 0.0, 0.0], dtype=np.float64)
        ...
        (out_constraint, final_ret, out_commands, out_power, first_slot, last_slot, min_idx, max_idx, _) = (
        ...
        assert total_energy > 0, f"ASAP constraint should get some power\n  Allocated: {total_energy:.1f}Wh"
        ...
        for i in range(num_slots):
            ...
            assert allocated <= available + 10, (
                f"Slot {i}: Power exceeds available\n  Allocated: {allocated:.1f}W\n  Available: {available:.1f}W"
            )
        ...
        print("✅ ASAP compute_best_period test passed!")
    ...
    def test_compute_best_period_price_optimization(self):
        ...
        prices = np.array(
            [0.1 / 1000, 0.4 / 1000, 0.1 / 1000, 0.4 / 1000, 0.1 / 1000, 0.4 / 1000, 0.1 / 1000, 0.4 / 1000],
            dtype=np.float64,
        )
        prices_ordered_values = [0.1 / 1000, 0.4 / 1000]

        (out_constraint, final_ret, out_commands, out_power, first_slot, last_slot, _, _, _) = (
            constraint.compute_best_period_repartition(
        ...
        cheap_energy = sum((out_power[i] * power_slots_duration_s[i]) / 3600.0 for i in cheap_slots)
        expensive_energy = sum((out_power[i] * power_slots_duration_s[i]) / 3600.0 for i in expensive_slots)
        ...
        assert cost < worst_case_cost, (
            f"Cost not optimized\n  Actual cost: {cost:.4f}€\n  Worst case: {worst_case_cost:.4f}€"
        )

        print("✅ Price optimization test passed!")
...
class TestConstraintAutoCommands:
    ...
    def test_auto_green_only_caps_to_available(self):
        ...
        power_steps = [LoadCommand(command="ON", power_consign=a * 230 * 3) for a in range(7, 33)]
        ...
        (out_constraint, final_ret, out_commands, out_power, _, _, _, _, _) = (
            constraint.compute_best_period_repartition(
                do_use_available_power_only=True,
                power_available_power=power_available_power,
                power_slots_duration_s=power_slots_duration_s,
        ...
        has_auto_commands = any(
            cmd
            and cmd.command in [CMD_AUTO_GREEN_ONLY.command, CMD_AUTO_FROM_CONSIGN.command, CMD_AUTO_GREEN_CAP.command]
            for cmd in out_commands
            if cmd
        )

        assert has_auto_commands, "support_auto=True should generate auto commands"
        ...
        print("✅ Auto command adaptation test passed!")
    ...
    def test_auto_green_cap_limits_consumption(self):
        ...
        power_slots_duration_s = np.array([SOLVER_STEP_S] * 4, dtype=np.float64)
        existing_commands = [LoadCommand(command="ON", power_consign=6000.0) for _ in range(4)]
        ...
        if has_changes:
            ...
            has_cap = any(cmd and cmd.is_like(CMD_AUTO_GREEN_CAP) for cmd in out_commands if cmd)
...
class TestConstraintEdgeCases:
    ...
    def test_constraint_at_exact_target_value(self):
        ...
        assert constraint.is_constraint_met(time), "Constraint at exact target should be met"
        ...
        assert abs(quantity_to_add) < 1.0, f"Should request no additional energy\n  Requested: {quantity_to_add}"

        print("✅ Exact target edge case passed!")
    ...
    def test_constraint_past_end_date_marked_met(self):
        ...
        assert constraint.is_constraint_met(time), "Constraint past end_of_constraint should be met"

        print("✅ Past end date edge case passed!")
    ...
    def test_constraint_with_zero_target(self):
        ...
        assert constraint.is_constraint_met(time), "Zero target constraint should be met"

        quantity = constraint.get_quantity_to_be_added_for_budgeting()
        assert abs(quantity) < 0.1, f"Zero target should request no energy\n  Requested: {quantity}"

        print("✅ Zero target edge case passed!")
    ...
    def test_constraint_with_target_less_than_current(self):
        ...
        assert constraint.is_constraint_met(time), "Constraint with current > target should be met"
        ...
        print("✅ Current > target edge case passed!")
