...
def test_create_ha_switch_includes_pool_branch():
    ...
    mock_pool.__class__ = QSPool
    with (
        patch("custom_components.quiet_solar.switch.create_ha_switch_for_QSPool", return_value=[]) as pool_mock,
...
def test_create_ha_switch_for_load(hass: HomeAssistant, load_config_entry: MockConfigEntry):
...
def test_create_ha_switch_for_load_no_green_only(hass: HomeAssistant, load_config_entry: MockConfigEntry):
...
@pytest.mark.asyncio
async def test_async_setup_entry(hass: HomeAssistant, switch_platform_config_entry: MockConfigEntry):
...
@pytest.mark.asyncio
async def test_async_unload_entry_handles_exception(hass: HomeAssistant, switch_platform_config_entry: MockConfigEntry):
