...
class TestConstraintPriorityAllocation:
    ...
    def test_constraint_priority_with_limited_solar(self):
        ...
        # Car: mandatory
        car_steps = [LoadCommand(command="ON", power_consign=a * 230 * 3) for a in range(7, 20)]  # Reduced range
    ...
    def test_asap_constraint_gets_highest_priority(self):
        ...
        print("✅ ASAP priority test passed!")
    ...
    def test_mandatory_vs_optional_with_battery(self):
        ...
        battery = create_test_battery(
            capacity_wh=6000.0, initial_soc_percent=50.0, min_soc_percent=15.0, max_soc_percent=90.0
        )
        ...
        mandatory_energy = calculate_energy_from_commands(commands_by_load.get("mandatory", []), end_time)
        optional_energy = calculate_energy_from_commands(commands_by_load.get("optional", []), end_time)
        ...
        print("✅ Mandatory vs optional with battery test passed!")
        print(f"   - Mandatory score: {mandatory_score}")
        print(f"   - Optional score: {optional_score}")
        print("   - Mandatory has priority: ✓")
    ...
    def test_before_battery_vs_after_battery_constraints(self):
        ...
        # =================================================================
        # Find final constraint states
        constraint_before_final = next(
            (c for c in solver._constraints_for_test if c.load.name == "before_battery"), None
        )
        ...
        before_progress = (
            constraint_before_final.current_value / constraint_before_final.target_value
            if constraint_before_final.target_value > 0
            else 0
        )
        after_progress = (
            constraint_after_final.current_value / constraint_after_final.target_value
            if constraint_after_final.target_value > 0
            else 0
        )
        ...
        battery_charge = reconstruct_battery_charge_evolution(
            initial_charge=battery._current_charge_value, battery_power=battery_power, durations_s=solver._durations_s
        )
        validate_battery_soc_bounds(battery_charge, battery)

        print("✅ Before/after battery test passed!")
...
class TestSolverComplexScenarios:
    ...
    def test_three_loads_different_priorities_realistic_day(self):
        ...
        battery = create_test_battery(
            capacity_wh=8000.0, initial_soc_percent=50.0, min_soc_percent=20.0, max_soc_percent=90.0
        )
        ...
        assert (
            car_satisfied or heater_satisfied or (car_final and car_final.current_value >= car_final.target_value * 0.8)
        ...
        print("✅ Complex 3-load scenario passed!")
        print(f"   - Car: {'met' if car_satisfied else 'partial'}")
        print(f"   - Heater: {'met' if heater_satisfied else 'partial'}")
        print("   - Battery SOC bounds: ✓")
        print("   - Energy conservation: ✓ (with_self_test)")
