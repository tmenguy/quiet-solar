...
class TestForecast(TestCase):
    def test_read_solar(self):
        ...
        assert (
            solar_provider.solar_forecast[-1][0] - solar_provider.solar_forecast[0][0]
        ).total_seconds() >= FLOATING_PERIOD_S
    ...
    def test_consumption_data_storage(self):
        ...
        async def _async_test():
            ...
            assert t == datetime(
                year=2024, month=8, day=20, hour=10, minute=0, second=0, microsecond=0, tzinfo=pytz.UTC
            )
    ...
    def test_read_passed_consumption_data(self):
        ...
        async def _async_test():
            ...
            conso_forecast = conso.home_non_controlled_consumption.compute_now_forecast(
                time_now=time, history_in_hours=24, future_needed_in_hours=FLOATING_PERIOD_S // 3600
            )
            ...
            solar_forecast = solar_provider.get_forecast(
                start_time=time, end_time=time + timedelta(seconds=FLOATING_PERIOD_S)
            )
            ...
            for j in range(3):
                ...
                charger = TestLoad(
                    name="charger", min_a=7, max_a=32, min_p=steps[0].power_consign, max_p=steps[-1].power_consign
                )

                cumulus = TestLoad(name="cumulus", min_a=6.5, max_a=6.5, min_p=1500, max_p=1500)
