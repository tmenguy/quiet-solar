...
from custom_components.quiet_solar.ha_model.charger import QSChargerGroup, QSChargerStatus, QSChargerWallbox
from custom_components.quiet_solar.ha_model.dynamic_group import QSDynamicGroup

# Import the necessary classes
from custom_components.quiet_solar.ha_model.home import QSHome
from custom_components.quiet_solar.home_model.commands import CMD_AUTO_FROM_CONSIGN, CMD_AUTO_GREEN_ONLY
...
class TestChargersSetup(unittest.IsolatedAsyncioTestCase):
        ...
        def is_current_acceptable_and_diff_mock(new_amps, estimated_current_amps, time):
            ...
            for i in range(3):
                if new_amps[i] > self.wallboxes_group.dyn_group_max_phase_current_conf:
                    return (
                        False,
                        [new_amps[i] - self.wallboxes_group.dyn_group_max_phase_current_conf for i in range(3)],
                    )
            return (True, [0, 0, 0])
    ...
    async def test_budgeting_algorithm_minimize_diffs_basic(self):
        ...
        with patch.object(self.wallboxes_group, "is_current_acceptable") as mock_acceptable:
            ...
            result, _, _ = await self.charger_group.budgeting_algorithm_minimize_diffs(
                actionable_chargers, full_available_home_power, full_available_home_power, False, self.current_time
            )
    ...
    async def test_budgeting_algorithm_phase_distribution(self):
        ...
        with patch.object(self.wallboxes_group, "is_current_acceptable") as mock_acceptable:
            ...
            result, _, _ = await self.charger_group.budgeting_algorithm_minimize_diffs(
                actionable_chargers, 10000.0, 10000.0, False, self.current_time
            )
...
class TestBudgetingAlgorithm:
    ...
    async def test_budgeting_algorithm_power_constraint(self):
        ...
        for i, (charger, config) in enumerate(zip(self.chargers, configs)):
            cs = self.create_charger_status(
                charger=charger, current_amp=config["current_amp"], power=config["power"], score=config["score"], idx=i
            )
            actionable_chargers.append(cs)
    ...
    async def test_budgeting_algorithm_zero_power(self):
        ...
        actionable_chargers = []
        for i, charger in enumerate(self.chargers):
            cs = self.create_charger_status(charger=charger, current_amp=10, power=2300.0, score=i + 1, idx=i)
            actionable_chargers.append(cs)
    ...
    async def test_budgeting_algorithm_high_solar_respects_group_limit(self):
        ...
        for iteration in range(max_iterations):
            ...
            print(
                f"  Phase currents: L1={total_phase_currents[0]}A, L2={total_phase_currents[1]}A, L3={total_phase_currents[2]}A"
            )

            result, _, _ = await self.charger_group_3p.budgeting_algorithm_minimize_diffs(
                current_chargers, full_available_home_power, full_available_home_power, False, self.current_time
            )
            ...
            print(
                f"  Budgeted phase currents: L1={budgeted_phase_currents[0]}A, L2={budgeted_phase_currents[1]}A, L3={budgeted_phase_currents[2]}A"
            )

            # Check if we've reached a stable state (no changes in budgeted amps)
            if iteration > 0 and all(
                results[-1][i].budgeted_amp == results[-2][i].budgeted_amp for i in range(len(results[-1]))
            ):
        ...
        print(
            f"Power utilization: {(total_final_power / full_available_home_power) * 100:.1f}% of available {full_available_home_power}W"
        )
