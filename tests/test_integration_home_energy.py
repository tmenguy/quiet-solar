...
class TestCompleteHomeEnergy:
    ...
    def test_complete_home_with_car_heater_pool_battery(self):
        ...
        pv_forecast = build_realistic_solar_forecast(
            start_time=start_time, num_hours=16, peak_power=9000.0, sunrise_hour=6, sunset_hour=20
        )

        # Realistic consumption
        ua_forecast = build_realistic_consumption_forecast(
            start_time=start_time, num_hours=16, base_load=500.0, peak_evening_load=1500.0, night_load=300.0
        )
    ...
    def test_off_grid_mode_battery_only_survival(self):
        ...
        battery = create_test_battery(
            capacity_wh=8000.0, initial_soc_percent=60.0, min_soc_percent=20.0, max_soc_percent=90.0
        )
        ...
        # =================================================================
        commands_by_load = {load.name: cmds for load, cmds in load_commands}
        lights_energy = calculate_energy_from_commands(commands_by_load.get("lights", []), end_time)
        ...
        print("   - Battery preserved for base load: ✓")
    ...
    def test_variable_pricing_shifts_loads_to_cheap_periods(self):
        ...
        battery = create_test_battery(
            capacity_wh=6000.0, initial_soc_percent=50.0, min_soc_percent=15.0, max_soc_percent=85.0
        )
        ...
        assert peak_energy <= normal_energy, (
            f"Should run less during peak periods\n  Normal: {normal_energy:.0f}W\n  Peak: {peak_energy:.0f}W"
        )
        ...
        print("   - Load shifted to cheaper periods: ✓")
    ...
    def test_multiple_constraints_same_load(self):
        ...
        battery = create_test_battery(capacity_wh=6000.0, initial_soc_percent=50.0)
        ...
        print("   - Solver handled correctly: ✓")
