...
class TestQSChargerGenericAdvanced(unittest.IsolatedAsyncioTestCase):
    ...
    def test_current_num_phases_no_switch(self):
        ...
        with (
            patch.object(charger, "can_do_3_to_1_phase_switch", return_value=False),
    ...
    def test_get_best_car_user_selection(self):
        ...
        charger.set_user_originated(USER_ORIGINATED_CAR_NAME, "UserSelectedCar")
    ...
    def test_get_best_car_automatic_selection(self):
        ...
        with (
            ...
            patch.object(charger, "get_car_score", side_effect=[100, 200]),
        ):  # Car2 has higher score
            # Mock available cars
    ...
    async def test_stop_charge(self):
        ...
        with (
            ...
            patch.object(
                type(charger), "_expected_charge_state", new_callable=PropertyMock
            ) as mock_expected_state_prop,
        ):
    ...
    async def test_start_charge(self):
        ...
        with (
            patch.object(charger, "is_charge_disabled", return_value=True),
    ...
    def test_check_plugged_val_with_car_confirmation(self):
        ...
        with (
            patch.object(charger, "get_last_state_value_duration", return_value=(None, None)),
    ...
    def test_is_charger_plugged_now_fallback(self):
        ...
        with (
            patch.object(charger, "get_car_plugged_in_status_vals", return_value=["Ready"]),
    ...
    async def test_set_charging_num_phases_no_switch(self):
        ...
        with (
            ...
            patch.object(type(charger), "physical_num_phases", new_callable=PropertyMock) as mock_physical,
            patch.object(
                type(charger), "_expected_num_active_phases", new_callable=PropertyMock
            ) as mock_expected_phases_prop,
        ):
            mock_physical.return_value = 3
    ...
    async def test_set_charging_num_phases_with_switch(self):
        ...
        with (
            patch.object(charger, "can_do_3_to_1_phase_switch", return_value=True),
    ...
    async def test_reboot(self):
        ...
        with (
            patch.object(charger, "can_reboot", return_value=True),
    ...
    async def test_reboot_cannot_reboot(self):
        ...
        with (
            patch.object(charger, "can_reboot", return_value=False),
            patch.object(charger, "low_level_reboot") as mock_reboot,
        ):
            await charger.reboot(time)
