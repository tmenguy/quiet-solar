...
def validate_energy_conservation(
    ...
    battery_power: npt.NDArray[np.float64] | None,
...
def calculate_total_cost(
    power: npt.NDArray[np.float64], durations_s: npt.NDArray[np.float64], prices: npt.NDArray[np.float64]
) -> float:
    """
...
def calculate_energy_from_commands(commands: list, end_time, slot_duration_s: float = 3600.0) -> float:
    """
...
def validate_constraint_satisfaction(
    constraint, commands: list, end_time, tolerance_percent: float = 5.0
...
def validate_power_limits(commands: list, min_power: float = 0.0, max_power: float | None = None) -> bool:
    """
...
def reconstruct_battery_charge_evolution(
    initial_charge: float, battery_power: npt.NDArray[np.float64], durations_s: npt.NDArray[np.float64]
...
def validate_no_overallocation(
    allocated_energies: dict[str, float], available_energy: float, tolerance_wh: float = 100.0
) -> None:
    """
