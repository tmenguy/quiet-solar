...
def build_realistic_solar_forecast(
    start_time: datetime, num_hours: int, peak_power: float = 8000.0, sunrise_hour: int = 6, sunset_hour: int = 18
...
def create_car_with_power_steps(
    min_amps: int = 7, max_amps: int = 32, num_phases: int = 3, voltage: float = 230.0, name: str = "car"
...
def create_simple_heater_load(power: float = 2000.0, name: str = "test_heater") -> TestLoad:
    """
...
def build_alternating_solar_pattern(
    start_time: datetime, num_hours: int, high_power: float = 5000.0, low_power: float = 500.0, period_hours: int = 2
) -> list[tuple[datetime, float]]:
    """
...
def calculate_total_available_energy(
    solar_forecast: list[tuple[datetime, float]], consumption_forecast: list[tuple[datetime, float]]
) -> float:
    """
