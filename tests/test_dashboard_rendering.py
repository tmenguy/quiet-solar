...
class TestDashboardTemplateRendering:
    ...
    async def test_solar_entities_appear_in_rendered_output(self, hass, full_dashboard_home):
        ...
        # using the translated name slugs (without the qs_ prefix used in description keys).
        assert "solar_forecast_ok" in rendered
        assert "forecast_age" in rendered
        # Dynamic entities created from Solcast provider
        assert "score_solcast" in rendered
        assert "solar_provider" in rendered
