...
The `_buildQRSPath`, `_chargingPowerToEcgAmp`, and
`_chargingPowerToEcgSpeed` instance methods are defined immediately
after the constructor — before `_startAnimation` — so they appear
in source order before their RAF-step call sites; the
`_extract_js_function_body` sentinel walker (in
`tests/test_dashboard_rendering.py`) finds the FIRST regex match,
so a definition-before-call layout is required for the AC-4 /
AC-5 sentinels to inspect the actual function bodies.
