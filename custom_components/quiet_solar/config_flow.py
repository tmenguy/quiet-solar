...
def selectable_power_entities(hass: HomeAssistant, domains=None) -> list:
    ...
    entities = [
        ...
        if ent.attributes.get(ATTR_UNIT_OF_MEASUREMENT) in UnitOfPower and ent.domain in ALLOWED_DOMAINS
    ]
...
def selectable_amps_entities(hass: HomeAssistant, domains=None) -> list:
    ...
    entities = [
        ent.entity_id
        for ent in hass.states.async_all(ALLOWED_DOMAINS)
        if ent.attributes.get(ATTR_UNIT_OF_MEASUREMENT) in UnitOfElectricCurrent and ent.domain in ALLOWED_DOMAINS
    ]
    return _filter_quiet_solar_entities(hass, entities)
...
def selectable_calendar_entities(hass: HomeAssistant, domains=None) -> list:
    ...
    if domains is None:
        ...
        ALLOWED_DOMAINS = domains
    entities = [ent.entity_id for ent in hass.states.async_all(ALLOWED_DOMAINS) if ent.domain in ALLOWED_DOMAINS]
...
def selectable_temperature_entities(
    ...
    entities = [
        ent.entity_id
        for ent in hass.states.async_all(ALLOWED_DOMAINS)
        if ent.attributes.get(ATTR_UNIT_OF_MEASUREMENT) in UnitOfTemperature and ent.domain in ALLOWED_DOMAINS
    ]
    return _filter_quiet_solar_entities(hass, entities)
...
def selectable_percent_sensor_entities(
    ...
    entities = [
        ent.entity_id
        for ent in hass.states.async_all(ALLOWED_DOMAINS)
        if ent.attributes.get(ATTR_UNIT_OF_MEASUREMENT) == PERCENTAGE and ent.domain in ALLOWED_DOMAINS
    ]
    return _filter_quiet_solar_entities(hass, entities)
...
def selectable_length_sensor_entities(
    ...
    entities = [
        ent.entity_id
        for ent in hass.states.async_all(ALLOWED_DOMAINS)
        if ent.attributes.get(ATTR_UNIT_OF_MEASUREMENT) in UnitOfLength and ent.domain in ALLOWED_DOMAINS
    ]
    return _filter_quiet_solar_entities(hass, entities)
...
class QSFlowHandlerMixin(config_entries.ConfigEntryBaseFlow if TYPE_CHECKING else object):
    ...
    def add_entity_selector(self, sc_dict, key, is_required, entity_list=None, domain=None):
        ...
        if entity_list:
            vals_sc = selector.EntitySelector(selector.EntitySelectorConfig(include_entities=entity_list))
        else:
            if domain:
    ...
    def get_common_schema(
        ...
        if default_section is not None:
            ...
            found_idx, options = map_section_selected_name_in_section_list(
                section_stored_name=good_value, section_list=options_raw_list, compute_options=True
            )
                    ...
                    sc.update(
                        {
                            vol.Required(
                                CONF_DEVICE_DYNAMIC_GROUP_NAME,
                                default=self.config_entry.data.get(CONF_DEVICE_DYNAMIC_GROUP_NAME, default_group),
                            ): SelectSelector(
                                SelectSelectorConfig(
                                    options=options,
                                    mode=SelectSelectorMode.DROPDOWN,
        ...
        if add_is_3p:
            sc.update(
                {
                    vol.Optional(CONF_IS_3P, default=self.config_entry.data.get(CONF_IS_3P, False)): cv.boolean,
                }
            )
        ...
        if add_phase_number:
            sc.update(
                {
                    vol.Optional(
                        CONF_MONO_PHASE, description={"suggested_value": self.config_entry.data.get(CONF_MONO_PHASE)}
        ...
        if add_boost_only:
            sc.update(
                {
                    vol.Optional(
                        CONF_LOAD_IS_BOOST_ONLY, default=self.config_entry.data.get(CONF_LOAD_IS_BOOST_ONLY, False)
                    ): cv.boolean,
                }
            )
            ...
            sc.update(
                {
                    vol.Required(
                        CONF_POWER,
                        description={
                            "suggested_value": self.config_entry.data.get(CONF_POWER, add_power_value_selector)
                        },
                    ): selector.NumberSelector(
                        selector.NumberSelectorConfig(
                            min=0,
                    ...
                    vol.Optional(
                        ...
                        description={
                            "suggested_value": self.config_entry.data.get(
                                CONF_DYN_GROUP_MAX_PHASE_AMPS, add_max_phase_amps_selector
                            )
                        },
                    ): selector.NumberSelector(
                        selector.NumberSelectorConfig(
        ...
        if add_load_power_sensor:
            ...
            if len(power_entities) > 0:
                self.add_entity_selector(
                    sc, CONF_ACCURATE_POWER_SENSOR, add_load_power_sensor_mandatory, entity_list=power_entities
                )
        ...
        if add_amps_sensors:
            ...
            if len(amps_entities) > 0:
                self.add_entity_selector(sc, CONF_PHASE_1_AMPS_SENSOR, False, entity_list=amps_entities)
                self.add_entity_selector(sc, CONF_PHASE_2_AMPS_SENSOR, False, entity_list=amps_entities)
                self.add_entity_selector(sc, CONF_PHASE_3_AMPS_SENSOR, False, entity_list=amps_entities)
            ...
            sc.update(
                {
                    vol.Optional(
                        CONF_DEVICE_EFFICIENCY,
                        description={"suggested_value": self.config_entry.data.get(CONF_DEVICE_EFFICIENCY, 100)},
                    ): selector.NumberSelector(
                        selector.NumberSelectorConfig(
                            min=1,
                            max=100,
        ...
        if add_mobile_app:
            ...
            if options:
                ...
                if default:
                    ...
                    keysc = vol.Optional(CONF_MOBILE_APP)
    ...
    def get_all_charger_schema_base(self, type, add_load_power_sensor_mandatory) -> tuple[dict, dict]:
        ...
        sc, placeholders = self.get_common_schema(
            type=type,
        ...
        if default is None:
            sc.update(
                {
                    vol.Optional(CONF_CHARGER_LATITUDE): vol.Maybe(vol.Coerce(float)),
                }
            )
        else:
            sc.update(
                {
                    vol.Optional(CONF_CHARGER_LATITUDE, description={"suggested_value": default}): vol.Maybe(
                        vol.Coerce(float)
                    ),
                }
            )
        ...
        if default is None:
            sc.update(
                {
                    vol.Optional(CONF_CHARGER_LONGITUDE): vol.Maybe(vol.Coerce(float)),
                }
            )
        else:
            sc.update(
                {
                    vol.Optional(CONF_CHARGER_LONGITUDE, description={"suggested_value": default}): vol.Maybe(
                        vol.Coerce(float)
                    ),
                }
            )
        ...
        sc.update(
            {
                vol.Optional(
                    CONF_CHARGER_MAX_CHARGE,
                    description={"suggested_value": self.config_entry.data.get(CONF_CHARGER_MAX_CHARGE, 32)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        step=1,
                ...
                vol.Optional(
                    CONF_CHARGER_MIN_CHARGE,
                    description={"suggested_value": self.config_entry.data.get(CONF_CHARGER_MIN_CHARGE, 6)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        step=1,
    ...
    async def async_step_home(self, user_input=None):
        ...
        sc_dict, placeholders = self.get_common_schema(
            type=TYPE, add_is_3p=True, add_max_phase_amps_selector=54, add_amps_sensors=True, add_load_power_sensor=True
        )
        ...
        sc_dict.update(
            {
                vol.Optional(
                    CONF_HOME_VOLTAGE,
                    description={"suggested_value": self.config_entry.data.get(CONF_HOME_VOLTAGE, 230)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        step=1,
            ...
            sc_dict.update(
                {
                    vol.Optional(
                        CONF_GRID_POWER_SENSOR_INVERTED,
                        default=self.config_entry.data.get(CONF_GRID_POWER_SENSOR_INVERTED, False),
                    ): cv.boolean,
                }
            )

        self.add_entity_selector(
            sc_dict, CONF_OFF_GRID_ENTITY, False, domain=[SENSOR_DOMAIN, BINARY_SENSOR_DOMAIN, SWITCH_DOMAIN]
        )
        ...
        sc_dict.update(
            {
                vol.Optional(
                    CONF_OFF_GRID_STATE_VALUE,
                    description={"suggested_value": self.config_entry.data.get(CONF_OFF_GRID_STATE_VALUE, "")},
                ): cv.string,
                vol.Optional(
                    CONF_OFF_GRID_INVERTED, default=self.config_entry.data.get(CONF_OFF_GRID_INVERTED, False)
                ): cv.boolean,
            }
        )
        ...
        sc_dict.update(
            {
                vol.Required(
                    CONF_HOME_PEAK_PRICE,
                    description={"suggested_value": self.config_entry.data.get(CONF_HOME_PEAK_PRICE, 0.27)},
                ): float,
                vol.Required(
                    CONF_HOME_OFF_PEAK_PRICE,
                    description={"suggested_value": self.config_entry.data.get(CONF_HOME_OFF_PEAK_PRICE, 0.2068)},
                ): float,
                vol.Required(
                    CONF_HOME_START_OFF_PEAK_RANGE_1,
                    description={
                        "suggested_value": self.config_entry.data.get(CONF_HOME_START_OFF_PEAK_RANGE_1, "00:00:00")
                    },
                ): selector.TimeSelector(),
                vol.Required(
                    CONF_HOME_END_OFF_PEAK_RANGE_1,
                    description={
                        "suggested_value": self.config_entry.data.get(CONF_HOME_END_OFF_PEAK_RANGE_1, "00:00:00")
                    },
                ): selector.TimeSelector(),
                vol.Required(
                    CONF_HOME_START_OFF_PEAK_RANGE_2,
                    description={
                        "suggested_value": self.config_entry.data.get(CONF_HOME_START_OFF_PEAK_RANGE_2, "00:00:00")
                    },
                ): selector.TimeSelector(),
                vol.Required(
                    CONF_HOME_END_OFF_PEAK_RANGE_2,
                    description={
                        "suggested_value": self.config_entry.data.get(CONF_HOME_END_OFF_PEAK_RANGE_2, "00:00:00")
                    },
                ): selector.TimeSelector(),
            }
        )
    ...
    async def async_step_solar(self, user_input=None):
        ...
        if len(power_entities) > 0:
            self.add_entity_selector(
                sc_dict, CONF_SOLAR_INVERTER_ACTIVE_POWER_SENSOR, False, entity_list=power_entities
            )
        ...
        if options:
            ...
            default = self.config_entry.data.get(CONF_SOLAR_FORECAST_PROVIDERS)
            if default is None:
        ...
        if default:
            sc_key = vol.Optional(CONF_SOLAR_MAX_OUTPUT_POWER_VALUE, description={"suggested_value": default})
        else:
        ...
        if default:
            sc_key = vol.Optional(CONF_SOLAR_MAX_PHASE_AMPS, description={"suggested_value": default})
        else:
    ...
    async def async_step_charger_ocpp(self, user_input=None):
        ...
        sc_dict[key_sc] = selector.DeviceSelector(selector.DeviceSelectorConfig(integration="ocpp"))
        schema = vol.Schema(sc_dict)
    ...
    async def async_step_charger_wallbox(self, user_input=None):
        ...
        sc_dict[key_sc] = selector.DeviceSelector(selector.DeviceSelectorConfig(integration="wallbox"))
        schema = vol.Schema(sc_dict)
    ...
    async def async_step_battery(self, user_input=None):
        ...
        sc_dict.update(
            {
                vol.Optional(
                    CONF_BATTERY_IS_DC_COUPLED, default=self.config_entry.data.get(CONF_BATTERY_IS_DC_COUPLED, False)
                ): cv.boolean,
            }
        )
        ...
        if len(number_entites) > 0:
            self.add_entity_selector(
                sc_dict, CONF_BATTERY_MAX_DISCHARGE_POWER_NUMBER, False, entity_list=number_entites
            )
        ...
        sc_dict.update(
            {
                vol.Required(
                    CONF_BATTERY_CAPACITY,
                    description={"suggested_value": self.config_entry.data.get(CONF_BATTERY_CAPACITY, 0)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        mode=selector.NumberSelectorMode.BOX,
                ...
                vol.Required(
                    CONF_BATTERY_MIN_CHARGE_PERCENT,
                    description={"suggested_value": self.config_entry.data.get(CONF_BATTERY_MIN_CHARGE_PERCENT, 0)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=0,
                        max=100,
    ...
    async def async_step_car(self, user_input=None):
        ...
        if user_input is not None:
            if "force_dampening" in user_input:
                ...
                charge_values_changed = (
                    user_input.get(CONF_CAR_CHARGER_MIN_CHARGE, 0) != min_charge
                    or user_input.get(CONF_CAR_CHARGER_MAX_CHARGE, 0) != max_charge
                )
                ...
                need_dampening_redisplay = (orig_dampening is False and new_dampening is True) or (
        ...
        sc_dict, placeholders = self.get_common_schema(
            type=TYPE, add_calendar=True, add_mobile_app=False, add_efficiency_selector=True
        )
        ...
        if len(num_percent_entity) > 0:
            ...
            if (
                self.config_entry.data.get(CONF_CAR_CHARGE_PERCENT_MAX_NUMBER_STEPS) is None
                or self.config_entry.data.get(CONF_CAR_CHARGE_PERCENT_MAX_NUMBER_STEPS) == ""
            ):
                sc_dict.update(
                    {
                        vol.Optional(CONF_CAR_CHARGE_PERCENT_MAX_NUMBER_STEPS): cv.string,
                    }
                )
        ...
        sc_dict.update(
            {
                vol.Optional(
                    CONF_CAR_BATTERY_CAPACITY,
                    description={"suggested_value": self.config_entry.data.get(CONF_CAR_BATTERY_CAPACITY, 0)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        mode=selector.NumberSelectorMode.BOX,
                ...
                vol.Optional(
                    CONF_CAR_CHARGER_MIN_CHARGE,
                    description={"suggested_value": self.config_entry.data.get(CONF_CAR_CHARGER_MIN_CHARGE, 6)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        step=1,
                ...
                vol.Optional(
                    CONF_CAR_CHARGER_MAX_CHARGE,
                    description={"suggested_value": self.config_entry.data.get(CONF_CAR_CHARGER_MAX_CHARGE, 32)},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        step=1,
                ...
                vol.Optional(
                    CONF_DEFAULT_CAR_CHARGE,
                    description={"suggested_value": int(self.config_entry.data.get(CONF_DEFAULT_CAR_CHARGE, 100))},
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=0,
                        max=100,
        ...
        if do_force_dampening is True or orig_dampening is True:
            sc_dict[
                vol.Optional(
                    CONF_CAR_IS_CUSTOM_POWER_CHARGE_VALUES_3P,
                    default=self.config_entry.data.get(CONF_CAR_IS_CUSTOM_POWER_CHARGE_VALUES_3P, False),
                )
            ] = cv.boolean
            ...
            for a in range(int(min_charge), int(max_charge) + 1):
                sc_dict[
                    vol.Optional(
                        f"charge_{a}", description={"suggested_value": self.config_entry.data.get(f"charge_{a}")}
                    )
                ] = selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=0,
    ...
    async def async_step_person(self, user_input=None):
        ...
        person_entities = [ent.entity_id for ent in self.hass.states.async_all([PERSON_DOMAIN])]
        ...
        if car_options:
            ...
            sc_dict.update(
                {
                    vol.Optional(
                        CONF_PERSON_AUTHORIZED_CARS, description={"suggested_value": default_authorized}
            ...
            if default_preferred:
                sc_dict.update(
                    {
                        vol.Optional(
                            CONF_PERSON_PREFERRED_CAR, description={"suggested_value": default_preferred}
                        ): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                                options=car_options,
                                mode=selector.SelectSelectorMode.DROPDOWN,
                ...
                sc_dict.update(
                    {
                        vol.Optional(CONF_PERSON_PREFERRED_CAR): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                                options=car_options,
                                mode=selector.SelectSelectorMode.DROPDOWN,
                            )
        ...
        if default_time:
            sc_dict.update(
                {
                    vol.Optional(
                        CONF_PERSON_NOTIFICATION_TIME, description={"suggested_value": default_time}
                    ): selector.TimeSelector()
                }
            )
        else:
            sc_dict.update({vol.Optional(CONF_PERSON_NOTIFICATION_TIME): selector.TimeSelector()})
    ...
    async def async_step_pool(self, user_input=None):
        ...
        for min_temp, max_temp, default in POOL_TEMP_STEPS:
            sc_dict[
                vol.Optional(
                    f"water_temp_{max_temp}",
                    description={"suggested_value": self.config_entry.data.get(f"water_temp_{max_temp}", default)},
                )
            ] = selector.NumberSelector(
                selector.NumberSelectorConfig(
                    min=-1,
    ...
    async def async_step_climate(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if "force_climate" in user_input:
                ...
                if (
                    user_input.get(CONF_CLIMATE) != orig_climate_entity
        ...
        if climate_entity is not None:
            ...
            sc_dict.update(
                {
                    vol.Required(
                        CONF_CLIMATE_HVAC_MODE_OFF, default=self.config_entry.data.get(CONF_CLIMATE_HVAC_MODE_OFF)
                    ): SelectSelector(
                        SelectSelectorConfig(
                            options=hvac_modes,
                            mode=SelectSelectorMode.DROPDOWN,
            ...
            sc_dict.update(
                {
                    vol.Required(
                        CONF_CLIMATE_HVAC_MODE_ON, default=self.config_entry.data.get(CONF_CLIMATE_HVAC_MODE_ON)
                    ): SelectSelector(
                        SelectSelectorConfig(
                            options=hvac_modes,
                            mode=SelectSelectorMode.DROPDOWN,
        ...
        if heat_pump_options:
            ...
            if default_heat_pump:
                sc_dict.update(
                    {
                        vol.Optional(
                            CONF_DEVICE_TO_PILOT_NAME, description={"suggested_value": default_heat_pump}
                        ): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                ...
                sc_dict.update(
                    {
                        vol.Optional(CONF_DEVICE_TO_PILOT_NAME): selector.SelectSelector(
                            selector.SelectSelectorConfig(
                                options=heat_pump_options,
                                mode=selector.SelectSelectorMode.DROPDOWN,
                            )
